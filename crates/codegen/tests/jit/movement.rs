...
fn reshape_2x4_to_4x2() {
    ...
    let ast = JitAST::new(BufferWithType::<[f32; 8]>::from(a_buf))
        .reshape(vec![4, 2])
        .reshape(vec![8]);
...
fn pad_2d() {
    ...
    let ast = JitAST::new(BufferWithType::<[f32; 4]>::from(a_buf))
        .reshape(vec![2, 2])
        .pad(vec![(1, 1), (1, 1)])
        .reshape(vec![16]);
...
fn shrink_2d() {
    ...
    let ast = JitAST::new(BufferWithType::<[f32; 16]>::from(a_buf))
        .reshape(vec![4, 4])
        .shrink(vec![(1, 1), (1, 1)])
        .reshape(vec![4]);
