...
impl RecompilationManager {
    ...
    pub fn recompile_all(&self, device: &Device) {
        self.shader_recompilation_handles.iter().for_each(|b| {
            b.send(RecompilationData {
                device: device.clone(),
                texture_format: self.curr_texture_format.unwrap(),
    ...
    pub fn register_shader<S>(
        &mut self,
        shader: Arc<ShaderWrapper<S>>,
        rx: Receiver<RecompilationData>,
    ) where
        S: for<'a> Shader<'a> + Send + 'static,
    {
        self.shader_recompilation_handles
            .push(shader.get_sender_tx());
        thread::spawn(move || {
